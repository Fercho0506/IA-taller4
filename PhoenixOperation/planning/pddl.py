...
def is_applicable(state: State, action: Action) -> bool:
    ...
    """
    # 1. Todas las precondiciones positivas deben ser un subconjunto del estado actual.
    if not action.precond_pos.issubset(state):
        return False
    
    # 2. Ninguna precondición negativa debe estar presente en el estado (deben ser disjuntos).
...
def apply_action(state: State, action: Action) -> State:
    """
    Return the new state: RESULT(s, a) = (s − DEL(a)) ∪ ADD(a)
    """
    # IMPORTANTE: El orden importa. Primero quitamos lo de del_list, luego sumamos add_list.
...
def get_applicable_actions(
    ...
    """
    Genera todas las acciones posibles y filtra las que se pueden ejecutar ahora.
    """
    # Obtenemos todas las combinaciones posibles (groundings)
    all_actions = get_all_groundings(domain, objects)
    
    # Filtramos usando nuestra función is_applicable
    return [action for action in all_actions if is_applicable(state, action)]
